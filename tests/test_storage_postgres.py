...
def test_pg_rebuild_action_index_reports_the_rows_it_corrected(storage: PostgresStorage) -> None:
    ...
    # Not a global drift() == 0 precondition: this suite shares one database
    # across tests, so a store-wide count can carry an earlier test's residue
    # and a rebuild below repairs that too, which is exactly why it is global.
