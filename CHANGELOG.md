...
- **`continuum attest-keygen` writes the private key 0600 instead of whatever
  ...
  secret, and any local user or backup copy could read raw key material and
  produce validly-signed attestations for a tampered chain. The private key is
  now created through `os.open` with mode 0600 and narrowed by `chmod`
  afterwards, because `os.open`'s mode only applies at creation and a
  pre-existing 0644 file being overwritten would keep its wider mode. The
  public key stays world-readable, as it should. The applied mode is now
  stated in both the human output and the JSON payload, so an operator on a
  filesystem without permission bits can see what they actually got. Such a
  filesystem keeps whatever it can express, the same compromise `save_budgets`
  makes for the budget registry — a private key landing weaker than a budget
  file was an inconsistency rather than a convention.
