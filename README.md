...
pytest -q                        # ~2,056 collected, ~2,030 passed, ~24 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 121 modules) plus a large test suite (154 test files, ~2,056 tests). All modules append to and replay one hash chained event log:
