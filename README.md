...
pytest -q                        # ~2,468 collected, ~2,440 passed, ~41 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 127 modules) plus a large test suite (178 test files, ~2,468 tests). All modules append to and replay one hash chained event log:
