...
pytest -q                        # ~2,457 collected, ~2,426 passed, ~28 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 124 modules) plus a large test suite (161 test files, ~2,457 tests). All modules append to and replay one hash chained event log:
