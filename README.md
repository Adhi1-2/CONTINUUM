...
pytest -q                        # ~2,486 collected, ~2,423 passed, ~28 skipped on a minimal env (exact counts vary)
...
CONTINUUM is one library (`src/continuum`, 124 modules) plus a large test suite (161 test files, ~2,486 tests). All modules append to and replay one hash chained event log:
