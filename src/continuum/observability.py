...
This module is a read-only renderer for the Phase 14 recovery dashboard built
from an existing :class:`~continuum.recovery.engine.RecoveryDecision`.

A process-wide metrics collector (``Metrics``, ``get_metrics``,
``collect_from_decision``, ...) used to live here but never had a product
caller: nothing incremented the counters and no surface read them, and the
recovery ledger (``continuum/recovery/ledger.py``) already answers the same
questions from durable history — including after a crash, the one situation
where in-memory counters would have been least trustworthy. It was removed in
issue #1032 and is recoverable from git history.
"""
